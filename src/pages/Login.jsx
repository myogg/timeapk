...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const user = existingUsers.find(user => user.username === username.trim());
      
      if (!user) {
        throw new Error('用户不存在，请先注册');
      }
      
      // 验证密码（在实际应用中应该验证加密后的密码）
      ...
      <div className="max-w-md w-full">
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl p-8 border border-white/20">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">智能工作记录系统</h1>
            <p className="text-blue-200">简单易用的工作记录工具</p>
          </div>
          ...
          <div className="mt-6 text-center">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white/10 text-blue-200">还没有账户？</span>
              </div>
            </div>
          </div>
